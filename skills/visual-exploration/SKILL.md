...
Browser-based visual exploration for showing mockups, layout options, and design directions before building.
...
**Use terminal** when the content is text or tabular:
...
skills/visual-exploration/scripts/start-server.sh --project-dir /path/to/project --foreground
```
...
```bash
skills/visual-exploration/scripts/start-server.sh \
...
The frame template provides classes for options (A/B/C choices), cards, mockup containers, split view, pros/cons, mock-element wireframe blocks, and typography helpers (`h2`, `h3`, `.subtitle`, `.section`, `.label`). Full catalog with markup examples for each: see [`references/css-class-catalog.md`](references/css-class-catalog.md). Consult when writing a mockup HTML file.
...
```bash
skills/visual-exploration/scripts/stop-server.sh $SCREEN_DIR
```
...
- Frame template (CSS reference): `skills/visual-exploration/scripts/frame-template.html`
- Helper script (client-side): `skills/visual-exploration/scripts/helper.js`
